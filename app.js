...
function showComparison(id) {
  ...
  const historyEntries=(Array.isArray(product.priceHistory)?product.priceHistory:[]).filter(entry=>window.PriceAlertRetailerCompliance.allows(entry.retailerId,"allowPriceHistory")&&validOfferPrice(entry.price)!==null);
