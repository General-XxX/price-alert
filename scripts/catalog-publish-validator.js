...
function validateProduct(product, metadata, index, enforceSourceAuthorization = false) {
  ...
  (product.offers || []).forEach((offer, offerIndex) => {
    ...
    if (!retailerCompliance.allows(offer,"allowAutomatedDataImport")) errors.push(`${offerPrefix}: retailer compliance does not permit automated catalog import.`);
    ...
    if (normalize(offer.affiliateUrl)) {
      if (!retailerCompliance.allows(offer,"allowAffiliateLink")) errors.push(`${offerPrefix}: retailer compliance does not permit affiliate links.`);
