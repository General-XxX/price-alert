...
function validOffer(product,offer,options={}){
  ...
  return Boolean(offer&&text(offer.offerId)&&text(offer.retailerId)&&current!==null&&/^[A-Z]{3}$/.test(text(offer.currency))&&available(offer)&&variantCompatible(product,offer,options.variantId)&&approved(offer,options));
}
...
function observation(product,offer,{timestamp=new Date().toISOString(),source="approved-catalog",sourceLastChecked=null}={}){
  if(!validOffer(product,offer,{mode:"development"}))return null;
